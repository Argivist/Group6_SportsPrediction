...
def main():
    ...
    release_clause_eur = st.text_input("release clause (eur)","0")
    dribbling = st.slider("dribbling", min_value=0, max_value=100, step=1)
    wage_eur =  st.text_input("wage (eur)", "0")
    power_shot_power = st.slider("power shot power", min_value=0, max_value=100, step=1)
    value_eur =  st.text_input("value (eur)", "0")
